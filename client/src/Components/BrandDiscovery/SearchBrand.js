...
const SearchBrand = () => {
	...
	useEffect(() => {
		...
		axios
			...
			.then((response) => {
				setRestaurants(response[0].data);
				setLoading(false);
			})
			.catch((error) => {
	...
	let dataSearch = restaurants.filter((item) => {
		const searchBranName = item.brandName
			.toString()
			.toLowerCase()
			.includes(searchName);
		const searchBranCity = item.country
			.toString()
			.toLowerCase()
			.includes(searchCity);
		const searchByFollowers = Object.values(item.socialMedia).some(
			(app) => app.numberOfFollowers >= followers
		);
					...
					<form className="form-group" id="form-sidebar">
						<label>
							<input
								...
								value="Beauty"
								name="name"
								onChange={handleCheckBox("Beauty", setCountBeauty)}
							/>
							<span>Beauty</span>
						...
						<label>
							<input
								...
								name="name"
								value="Fashion"
								onChange={handleCheckBox("Fashion", setCountFashion)}
							/>
							<span>Fashion</span>
							<span className="num" dataVal="400">
								{countFashion}
						...
						<label>
							<input
								...
								name="name"
								value="Sports"
								onChange={handleCheckBox("Sports", setCountSports)}
							/>
							<span>Sports</span>
							<span className="num" dataVal="400">
								{countSports}
						...
						<label>
							<input
								...
								name="name"
								value="Technology"
								onChange={handleCheckBox("Technology", setCountTechnology)}
							/>
						...
						<label>
							<input
								...
								name="name"
								value="Food"
								onChange={handleCheckBox("Healthcare", setCountHealthcare)}
							/>
						...
						<label>
							<input
								...
								name="name"
								value="Food"
								onChange={handleCheckBox("Food", setCountFood)}
							/>
							<span>Food</span>
							<span className="num" dataVal="400">
								{countFood}
						...
						<label>
							<input
								...
								name="name"
								value="Travel"
								onChange={handleCheckBox("Travel", setCountTravel)}
							/>
							<span>Travel</span>
							<span className="num" dataVal="400">
								{countTravel}
						...
						<label>
							<input
								...
								name="name"
								value="Education"
								onChange={handleCheckBox("Education", setCountEducation)}
							/>
							<span>Education</span>
							<span className="num" dataVal="400">
								{countEducation}
