...
export default function BlogPost() {
  ...
  useEffect(() => {
    setTitle('Finibus Bonorum et Malorum');
    setAuthor('John Doe');
    setDate('Monday, May 20');
    setImage(picture);
    setParagraphs([
           ...
           consectetur, adipisci velit, sed quia non numquam eius modi tempora incidunt ut labore et
            dolore magnam aliquam quaerat voluptatem. Ut enim ad minima veniam,
             quis nostrum exercitationem ullam corporis suscipit laboriosam, nisi ut aliquid ex
              ...
              velit esse quam nihil molestiae consequatur, vel illum qui dolorem eum fugiat qu voluptas nulla pariatur?"`,
      `1914 translation by H. Rackham"But I must explain to you how all this mistaken idea 
  ...
  return (
    <div className="blog-posts">
      <div className="picture">
        <img className='picture' src={image} alt="Blog Post" />
      </div>
      <div className="finibus-bonorum-et-malorum">
        {title}
      </div>
      <div className="container">
        <span className="written-by-john-doe">
          Written by {author}
        </span>
        <span className="monday-may-20">
          {date}
        </span>
      </div>
      ...
      <div className="cards-container">
        {paragraphs.map((paragraph, index) => (
          <Card key={index} content={paragraph} />
        ))}
      </div>
    </div>
  );
